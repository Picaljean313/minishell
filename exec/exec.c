...
/*   Updated: 2025/08/05 16:49:25 by anony            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	// if (fds->fdin != -1)
	// 	if (dup2(fds->fdin, STDIN_FILENO) == -1)
	// 		return (perror("dup2"), ft_close_fd(&fds->fdin), 1);
	// if (fds->fdout != -1)
	// 	if (dup2(fds->fdout, STDOUT_FILENO) == -1)
	// 		return (perror("dup2"), ft_close_fd(&fds->fdout), 1);
	// if (fds->fderr != -1)
	// 	if (dup2(fds->fderr, STDERR_FILENO) == -1)
	// 		return (perror("dup2"), ft_close_fd(&fds->fderr), 1);
	if (dup2(fds->fdin, STDIN_FILENO) == -1)
